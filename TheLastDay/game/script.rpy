...
define a = Character("Анастасия", who_color="#CD853F", what_color="#2c1e0e")
define o = Character("Ольга", who_color="#556B2F", what_color="#2c1e0e")
define t = Character("Татьяна", who_color="#4682B4", what_color="#2c1e0e")
define m = Character("Мария", who_color="#D2691E", what_color="#2c1e0e")
define n = Character("Николай", who_color="#2F4F4F", what_color="#2c1e0e")
...
define g = Character("Охранник", who_color="#A0522D", what_color="#2c1e0e")
define b = Character("Доктор Боткин", who_color="#708090", what_color="#2c1e0e")
define h = Character("Харитонов", who_color="#708090", what_color="#2c1e0e")
define d = Character("Демидова", who_color="#708090", what_color="#2c1e0e")




init python:
    def wait_for_key_qte(target_key, timeout=3.0):
        """Ожидает нажатия конкретной клавиши в течение timeout секунд.
        Возвращает True если нажали правильно, False если время вышло или нажали не ту клавишу."""
        ...
        while time.time() - start < timeout:
            ...
            if renpy.display.interface.event.peek():
                ev = renpy.display.interface.event.get()
                if ev.type == renpy.display.event.KEYDOWN:
                    key = ev.key
                    # Сопоставляем коды клавиш
                    if key == pygame.K_a and target_key == "A":
                        return True
                    elif key == pygame.K_s and target_key == "S":
                        return True
                    elif key == pygame.K_d and target_key == "D":
                        return True
                    elif key == pygame.K_f and target_key == "F":
                        return True
                    elif key == pygame.K_j and target_key == "J":
                        return True
                    elif key == pygame.K_k and target_key == "K":
                        return True
                    elif key == pygame.K_l and target_key == "L":
                        return True
                    elif key == pygame.K_SEMICOLON and target_key == ";":
                        return True
                    elif key == pygame.K_ESCAPE:
                        # Не даём выйти через Esc
                        continue
                    else:
                        return False  # Нажали не ту клавишу
            renpy.ui.interact(suppress_overlay=True, suppress_underlay=True, roll_forward=None)
...
label morning:
    ...
    "На походных кроватях спят княжны Романовы. Самая старшая - Ольга расположилась в дальнем углу комнаты, 
    на кровати рядом с ней - Татьяна, у двери - Мария и, наконец, у окна лежит уже проснувшаяся Анастасия."
    "Внутренний голос Анастасии" "Сегодня мне снился Тобольск..."
    "Внутренний голос Анастасии" "Вот наш дом, а чуть подле - заснеженный Иртыш."
    "Внутренний голос Анастасии" "Папа во дворе колит дрова, а мы с Алёшкой лепим снежную бабу..."
    "Внутренний голос Анастасии" "Сёстры опять помогают матушке на кухне."
    "Внутренний голос Анастасии" "Скоро Рождество… Так странно… Почему мне сниться зима? Ведь сейчас июль..."
...
label examine_room_morning:
    ...
    menu:
        "Осмотреть тумбочку":
            ...
            "На тумбочке лежит небольшая икона Спаса Нерукотворного. Анастасия креститься, 
            нашёптывая короткую молитву себе под нос, и берёт найденный предмет в руки."
            "Внутренний голос Анастасии" "Эту икону мама дала мне перед отъездом из Царского Села. 
            Как же давно это было… Кажется, с того момента прошла целая вечность."
            "Внутренний голос Анастасии" "Негоже иконе на тумбочке лежать. Спрячу ка я её куда понадёжнее..."
        ...
        "Посмотреть в окно":
            ...
            window show
            "Сквозь забеленные стёкла почти ничего не видно. 
            Анастасия с трудом разглядывает в этой белизне размытые силуэты деревьев и небольшой кусочек неба."
            "Внутренний голос Анастасии" "А ведь раньше, в Царском Селе, я могла часами разглядывать живописные 
            пейзажи. Я помню наш Александровский дворец, аккуратный и светлый, наши пруды с лебедями, парк 
            невообразимых размеров... Жаль, что не могу насладиться этой красотой хоть ещё разок..."
            "Она тянется рукой к холодному стеклу... касается... 
            но резко одёргивает её, оглядываясь на дверь - не вошёл ли часовой."
            window hide
        ...
        "Посмотреть на спящих сестер":
            ...
            "Внутренний голос Анастасии" "Оля как вторая мама. Только мама устала, а Оля держится. Ради нас."
            "Татьяна поджала ноги к груди, свернулась калачиком. Даже во сне изящная, аккуратная."
            "Внутренний голос Анастасии" "Таня всегда говорила, что так теплее. Наверное, ей холодно. Нам всем холодно последнее время."
    ...
    # После любого осмотра возвращаемся к кровати
    jump go_to_the_bed
...
label wake_olga:
    ...
    "Анастасия ещё несколько мгновений смотрит на сестру. По лицу той видно, что спалось ей плохо, 
    хотя она старается не подавать виду."
    ...
    "Оля слегка улыбается. Прислушивается. В коридоре ходят то ли слуги, то ли охрана. 
    ...
    o "Сестрёнка, сейчас примерно половина восьмого. Не стоит беспокоиться. Я бы обязательно вас разбудила. 
    Но ты умница. Лучше лишний раз проверить, чем потом жалеть."
    ...
    "Неожиданно для себя Анастасия чувствует облегчение. На её лице уже сверкает яркая улыбка, 
    а в глазах заметен свойственный её глазам блеск."
    ...
    "Оля окончательно просыпается и встаёт с кровати. 
    Она оглядывает Татьяну и Марию, которые всё ещё сладко спят. Старшая сестра пока не решается их будить."

    "Внутренний голос Ольги" "Пусть ещё поспят. Некуда нам торопиться..."
    "Взгляд Ольги снова падает на Анастасию. Та стоит в стороне - робкая, отстранённая, будто всё ещё 
    пытается проснуться."
...
label olga_question:
    ...
    menu:
        "«Мне снился сон...»":
            ...
            "Анастасия немного мешкается. Она не хочет беспокоить сестру своим странным сном. 
            Но всё же, немного подумав, решается сказать."
            ...
            a "Мы готовились к Рождеству. На кухне вкусно пахло едой... На улице зима. 
            И вроде должно быть холодно... а мне тепло..."
        ...
        "«Охрана всю ночь не давала спать»":
            ...
            "Внутренний голос Анастасии" "Что если они праздновали..."
            "Анастасия резко замотала головой."
            "Внутренний голос Анастасии" "Настя! Что за глупости! Возьми себя в руки. 
            Этого просто не может быть!"
       
            if inventory.has_item(1):
                "Несмотря на попытки успокоить себя, рука Анастасии невольно касается подушки, 
                под которой спрятана икона."
                "Ольга замечает это."
                ...
                a "Злая мысль пробежала в голове... Вот утром икону нашла... Ту, что мама дала. Лежала на тумбочке. 
                Я и подумала - она поможет справиться с этой мыслью..."
    ...
    "Внутренний голос Анастасии" "Мари такая веселушка! И сны у неё забавные. И сама она по-доброму смешная..."
    
    # Финальные мысли Анастасии
    "Анастасия смотрит на сестёр и чувствует тепло в груди."
    
    if inventory.count_found() >= 1:
        "Внутренний голос Анастасии" "Ну, теперь икона со мной. Сёстры рядом. Мама с папой за стеной. Оля права - пока мы вместе, нам ничего не страшно."
    else:
        "Внутренний голос Анастасии" "Мы выдержим это нелёгкое испытание. Таня права - осталось немного потерпеть. Ведь мы вместе. Мы - семья. А семье невзгоды не страшны."
...
label breakfast:
    ...
    # Проверка на наличие крестика Алексея (ID 7)
...
label lunch_scene:
    ...
    al "Пойдёмте, дети. Только недолго."
...
label walk_scene:
    ...
    #play audio "audio/ambient/footsteps.ogg" loop volume 0.3
    ...
    show maria
    "Мария подходит к забору, проводит пальцами по доскам."

    m "Интересно, что там? За этим забором?"

    a "Улица. Наверное, обычная. Пыльная. С собаками."
    ...
    menu:
        ...
        "Давай я подсажу тебя. Посмотришь одну секунду.":
            ...
            #stop audio
            #play sound "audio/guard_shout.ogg"
...
if not inventory.has_item(3):
    ...
    menu:
        ...
        "Давайте споём. Тихо. Ту песню, что пели в Тобольске. Маменька услышит в окно.":
            ...
            #play music "audio/music/tobolsk_song.ogg" fadein 2.0 volume 0.4
    ...
    if not inventory.has_item(4):
            ...
            menu:
                ...
                "Просто посмотреть, что сверху":
                    ...
                    "Внутренний голос Анастасии" "Слишком много воспоминаний. Они ещё не выветрились."
                    "Она возвращается к альбому с фотографиями."
                    
                "Не трогать коробку":
                    "Анастасия смотрит на коробку, но решает не трогать её. Пусть стоит, как стоит."
                    "Внутренний голос Анастасии" "Может, в другой раз. Когда будет легче."
...
label cards:
    ...
    menu:
        "Сказать правду" if inventory.count_found() >= 3:
...
label card_game_rules:
    ...
    show text "ПРАВИЛА ИГРЫ В БЕЗИК" at truecenter with dissolve
    $ renpy.pause(2.0)
    hide text
    ...
    if game_finished:
        ...
        hbox:
            ...
            for i, card in enumerate(player_hand):
                ...
                imagebutton:
                    idle Transform(f"images/cards/{card_name}.png", size=(120, 160))
                    hover Transform(f"images/cards/{card_name}.png", size=(130, 150))
        ...
        hbox:
            ...
            for card in current_trick:
                $ card_name = f"{card['rank']}_{card['suit']}"
                add Transform(f"images/cards/{card_name}.png", size=(120, 160))
        ...
        textbutton "Объявить комбинацию":
            ...
            hover_background "#CD853F"  # цвет при наведении
...
init python:
    ...
    def make_player_move():
        ...
        if selected_card is not None and waiting_for_player:
            ...
            renpy.restart_interaction()
            renpy.call_in_new_context("opponent_move")
    
    def opponent_move_logic():
        global opponent_hand, current_trick, player_score, opponent_score, waiting_for_player
        global deck_cards, player_hand, last_trick_info
        
        if opponent_hand:
            best_card = max(opponent_hand, key=lambda c: c["value"])
            ...
            if not player_hand and not opponent_hand and not deck_cards:
                renpy.call_in_new_context("end_round")
            elif not player_hand and not opponent_hand and deck_cards:
                if len(deck_cards) >= 8:
                    player_hand = deck_cards[:4]
                    opponent_hand = deck_cards[4:8]
                    deck_cards = deck_cards[8:]
                    add_to_log("Карты добраны! Новый раунд.")
    ...
    def check_combinations():
        global player_score, declared_combos_player
...
label end_round:
    python:
        global game_round, player_score, opponent_score, game_over, game_finished
        global deck_cards, player_hand, opponent_hand, declared_combos_player, declared_combos_opponent
        global last_trick_info, waiting_for_player
        ...
        if game_round < 3:
            ...
            if player_score > opponent_score:
                renpy.jump("card_game_player_wins")
            elif player_score < opponent_score:
                renpy.jump("card_game_opponent_wins")
            else:
                renpy.jump("card_game_tie")
...
label card_game_player_wins:
    ...
    if player_is_nikolai:
        ...
        ax "А в следующий раз я тоже сыграю!"
        al "Это всё практика, Таня. А теперь пора отдыхать."
...
if not inventory.has_item(7) and inventory.count_found() >= 6:
    ...
    menu:
        "Вернуться за крестиком":
            ...
            "Внутренний голос Анастасии" "Это крестик, который бабушка подарила. Лёша никогда с ним не расставался."
            ...
            if all_items_collected:
                "Внутренний голос Анастасии" "У каждого из нас теперь есть что-то святое... Мы взяли с собой самое дорогое. Что бы ни случилось — мы не одни."
